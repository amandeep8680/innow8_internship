...
class GroupChatMessage(AsyncWebsocketConsumer):
    async def connect(self):
        user = self.scope.get("user")
        print("User for group :", user)
        ...
        history = await self.group_history()
...
#######=################Receiver for group ########################################
    ...
    async def receive(self, text_data):
        ...
        await self.save_group_mesage(message)
    ...
    ## save message in the db of group

    @database_sync_to_async
    def save_group_mesage(self, message):
